#!/usr/bin/env bash
# ============================================================
#  Transcendence Campaign Modules — PDF Build Script
#  Usage:  ./build-modules.sh [en|es] ["Book Title"]
# ============================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PUBLICATIONS_DIR="$(dirname "$SCRIPT_DIR")"
BOOK_DIR="$PUBLICATIONS_DIR/core-books/transcendence-campaign-modules"
...
LANG="${1:-es}"
TITLE="${2:-Módulos de Campaña}"
AUTHOR="${3:-}"

mkdir -p "$OUTPUT_DIR"
HTML_OUT="$OUTPUT_DIR/transcendence-modules-${LANG}.html"
PDF_DEST="$BOOK_DIR/98-layout-export/${LANG}"
PDF_OUT="$PDF_DEST/transcendence-modules-${LANG}.pdf"
PDF_RAW="${TMPDIR:-/tmp}/transcendence-modules-${LANG}.raw.pdf"
PAPER_BACKGROUND="$ASSETS_DIR/corebook-paper-background.jpg"
mkdir -p "$PDF_DEST"
trap 'rm -f "$PDF_RAW"' EXIT

echo "============================================"
echo "  Building Transcendence Campaign Modules"
echo "  Language : $LANG"
echo "  Title    : $TITLE"
echo "============================================"
echo ""
