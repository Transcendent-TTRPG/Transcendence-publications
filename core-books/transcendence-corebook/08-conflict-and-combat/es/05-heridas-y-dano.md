...
Las heridas definen qué ocurre cuando un ataque supera la defensa. No reemplazan a la Tirada de Ataque ni a la Tirada de Impacto. Entran después, cuando ya existe contacto efectivo y la escena necesita saber qué parte del cuerpo, equipo o criatura soportó la consecuencia.

Transcendence separa dos superficies de daño. Cuando un personaje jugador golpea a un PNJ, criatura o monstruo, el ataque usa el modelo de daño del objetivo. Cuando un PNJ golpea a un personaje jugador, el golpe se registra como una herida localizada por zona.

Esta separación permite que los enemigos tengan HP, zonas, fases, partes rompibles o puntos vitales propios, mientras que los personajes jugadores no dependen de una barra general de vida. Un golpe recibido deja una marca física. Esa marca ocupa espacio, limita función y puede hacer colapsar una zona.
...
2. Se resuelve la Tirada de Ataque contra la Tirada de Defensa del objetivo.
3. Si la Tirada de Ataque no supera la Tirada de Defensa, el ataque no conecta de forma efectiva.
4. Si el ataque conecta, el atacante realiza la Tirada de Impacto.
5. Se determina el Bloqueo aplicable.
6. El resultado se convierte en daño, herida, ruptura o efecto según el objetivo.

La Tirada de Impacto no se realiza si el ataque no encuentra una entrada. La defensa decide si el golpe conecta. El Bloqueo decide cuánto de esa presión llega realmente al cuerpo, pieza o parte golpeada.
...
```text
Impacto = (Rango de Competencia x Daño del arma) + (Característica asociada x Grado del arma)
```

El Impacto no representa solo fuerza bruta. Incluye masa, ángulo, ejecución, calidad del arma, punto de contacto y capacidad del atacante para convertir una apertura en consecuencia.
...
Si la Tirada de Impacto usa varios dados, el atacante debe designar uno antes de tirar. En mesa, se usa un dado de color distinto. En digital, el dado puede marcarse con una etiqueta equivalente. Solo ese dado valida el Impacto Crítico; los demás dados suman Impacto de forma normal.
...
Las armas con dados pequeños generan críticos con más frecuencia. Las armas con dados grandes generan críticos menos frecuentes, pero suelen transmitir más Impacto. Subir de rango aumenta el Impacto total, pero no aumenta por sí mismo la probabilidad crítica, porque solo el dado designado puede validarla.
...
```text
Potencia Crítica = Potencia base x Multiplicador de Potencia del arma
```
...
El objetivo debe ser algo que el ataque pueda alcanzar y afectar de forma creíble: un arma, escudo, pieza de armadura, extremidad, mandíbula, cuerno, caparazón, cola, ala, articulación, punto vital o parte destructible definida por el encuentro.
...
La pérdida de Durabilidad solo ocurre durante un intento válido de ruptura: por Impacto Crítico, por una Técnica que permita romper sin crítico, o por una regla específica del ataque. Los ataques normales no reducen Durabilidad por defecto.
...
Romper una parte puede deshabilitar una opción de ataque, reducir defensa, destruir equipo, impedir una Técnica dependiente de esa parte, reducir movilidad, alterar un patrón de comportamiento, abrir un punto vulnerable o cambiar una fase del encuentro.
...
La Competencia Defensiva equivale al nivel de competencia en el tipo de armadura que protege la zona golpeada. Solo se usa si esa armadura participa realmente en absorber el Impacto.
...
3. Resolver la Tirada de Defensa con la Agilidad aplicable a esa armadura.
...
| --- | --- | ---: |
| Impacto <= Bloqueo x 1 | Sin herida | 0 |
| Impacto > Bloqueo x 1 y < Bloqueo x 2 | Herida Leve | 1 |
| Impacto >= Bloqueo x 2 y < Bloqueo x 3 | Herida Grave | 2 |
| Impacto >= Bloqueo x 3 | Herida Crítica | 3 |
...
Si la zona tiene suficientes ranuras libres, se marcan normalmente. Si no tiene suficientes ranuras libres, se marcan las ranuras restantes y el exceso produce Desbordamiento.
...
Un personaje tiene el Torso con 4 de 5 ranuras ocupadas. Recibe una Herida Grave en el Torso, que debería ocupar 2 ranuras. Solo queda 1 ranura libre, así que marca esa ranura y la otra produce Desbordamiento. El Torso queda Colapsado.
...
Este penalizador solo se aplica a tiradas y acciones que dependan claramente de esa zona. No es un penalizador universal al personaje.
...
| Pies | No puede esprintar. Penalizador de Saturación a Tiradas de Especialización físicas de Agilidad que dependan de apoyo fino. | Puede moverse solo con apoyo, ayuda o una Tirada de Especialización apropiada. Si intenta moverse bajo presión sin apoyo y falla, queda Derribado. |
...
Si una zona ya Colapsada recibe otra herida, se aplica o refresca el efecto de Colapso. Si la zona es Cabeza o Torso, el personaje debe superar una Tirada de Resistencia de Alteración contra la severidad de la nueva herida. Si falla, Cabeza escala hacia Inconsciente y Torso escala hacia Agonía.
...
Medicina cubre estabilizar, tratar y curar daño corporal. Herboristería, alquimia, objetos, Técnicas y artefactos pueden modificar estas reglas desde sus propias secciones.
...
En un éxito, la herida queda estabilizada. Sus efectos inmediatos dejan de empeorar. En un fallo, la herida sigue activa y el intento puede consumir tiempo, recursos o abrir una complicación si la escena sigue bajo presión.
...
Al tratar una zona durante un Descanso Completo, el personaje que atiende al paciente realiza una Tirada de Especialización de Medicina con Sabiduría. La dificultad base y el kit dependen de la ranura más severa que todavía esté ocupada en esa zona.
...
En un éxito, el paciente libera 1 ranura ocupada de esa zona. La ranura liberada debe pertenecer a una herida estabilizada. En un fallo, la zona no libera ranuras.

Cuando hay varias heridas estabilizadas en la misma zona, quien realiza el tratamiento declara qué ranura intenta liberar antes de tirar. El Narrador puede exigir que primero se libere una ranura de la herida más severa si esa lesión mantiene Agonía, Colapso o una pérdida funcional dominante.
...
Por defecto, una criatura importante debería organizarse en cinco lugares principales para mantener lectura en mesa. Esos lugares pueden ser cráneo, mandíbula, torso, patas y cola; núcleo, placas dorsales, extremidades, alas y cola; o cualquier distribución equivalente.
...
Romper una parte de criatura sirve para limitar opciones del enemigo. Si un lobo de hielo tiene Aliento Helado vinculado a su mandíbula, romper la mandíbula impide usar esa habilidad hasta que el bloque del enemigo diga lo contrario.
