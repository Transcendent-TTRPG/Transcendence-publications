---
title: "Catálogo de Armas"
type: corebook
content_kind: reference
writing_mode: reference
language: es
chapter: 10
status: draft
canonical: false
tags: [armas, equipo, catálogo, combate, lanzas, hachas, mazas, hojas, dagas, arrojadizas, a distancia, flexibles]
related:
  - core-books/transcendence-corebook/10-equipment-and-resources/es/04-fabricacion-equipo.md
  - core-books/transcendence-corebook/10-equipment-and-resources/es/08-catalogo-de-materiales.md
  - core-books/transcendence-corebook/10-equipment-and-resources/es/10-catalogo-de-armaduras.md
authority_refs:
  - Transcendence-design/data/system/combat-equipment-catalog.yaml
  - Transcendence-design/docs/system/combat-equipment-catalog.md
  - Transcendence-design/data/system/weapon-technique-profiles.yaml
---

# Catálogo de Armas

Este catálogo lista todas las armas disponibles organizadas por familia. La familia determina qué perfiles de técnica puede usar el arma. El material del que está fabricada el arma modifica su Durabilidad y Potencia, no su dado de daño ni su característica.

Cada entrada indica cuántas manos requiere, el dado de daño, el alcance, el peso aproximado, la característica asociada para las tiradas, y si es un arma primaria o auxiliar. El **bonificador** se aplica por cada rango de competencia completo en el tipo de arma.

Las armas **Primarias** son el arma principal de un combatiente. Las **Auxiliares** se requieren para combate con dos armas — sin un arma auxiliar no es posible ejecutar técnicas ni acciones de dos armas.

---

## Lanzas

Armas de asta larga diseñadas para el alcance. La mayoría requieren dos manos para aprovechar su longitud y su palanca.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Lancea | 2 | d12 | 2 m | 4 kg | Fuerza | Primaria | +1 por rango a T.A. contra enemigos montados o de tamaño Grande |
| Partisana | 2 | d10 | 2 m | 4,5 kg | Tenacidad | Primaria | +1 por rango a T.I. cuando se ataca desde el alcance máximo |
| Kontos | 2 | d12 | 2 m | 5 kg | Agilidad | Primaria | +1 por rango al daño contra enemigos sin escudo |
| Yari | 2 | d12 | 2 m | 4,2 kg | Astucia | Primaria | Ignora 1 punto de Bloqueo por rango |
| Hasta | 1 | d8 | 1 m | 2,5 kg | Astucia | Primaria | +1 por rango a T.D. cuando se usa para bloquear a corta distancia |
| Dory | 1 | d8 | 1 m | 2,3 kg | Agilidad | Primaria | Sube un grado de dado cuando se usa como arma arrojadiza |
| Ranseur | 2 | d10 | 2 m | 5,2 kg | Fuerza | Primaria | +1 por rango a T.A. durante una carga |

---

## Hachas

Armas pesadas diseñadas para abrir armaduras y cortar a través de defensas. Las versiones de dos manos tienen mayor alcance y dado de daño; las de una mano priorizan velocidad y versatilidad.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Guja | 2 | d10 | 2 m | 5 kg | Fuerza | Primaria | Ignora 1 punto de armadura por rango si el enemigo no lleva armadura pesada |
| Alabarda | 2 | d10 | 2 m | 4,5 kg | Tenacidad | Primaria | +1 por rango a T.I. cuando se ataca desde el alcance máximo |
| Naginata | 2 | d12 | 2 m | 4,5 kg | Astucia | Primaria | +1 por rango a T.I. cuando se usa para reacciones |
| Voulge | 2 | d12 | 2 m | 5,5 kg | Agilidad | Primaria | +1 a T.I. contra enemigos sin escudo |
| Bec de Corbin | 2 | d10 | 2 m | 4,8 kg | Fuerza | Primaria | +1 por rango a T.I. contra enemigos con armadura pesada |
| Pudao | 2 | d12 | 2 m | 5,3 kg | Sabiduría | Primaria | +1 por rango a T.A. por cada enemigo adyacente, máximo +3 |
| Sagaris | 1 | d8 | 1 m | 2 kg | Fuerza | Primaria | +1 por rango a T.I. contra enemigos con armadura ligera |
| Skeggox | 1 | d6 | 1 m | 1,5 kg | Fuerza | Auxiliar | +1 por rango a T.A. en combate con dos armas |
| Labrys | 2 | d12 | 2 m | 3,5 kg | Tenacidad | Primaria | Ignora 1 punto de armadura por rango en ataques a dos manos |
| Dolabra | 1 | d6 | 1 m | 2,5 kg | Tenacidad | Primaria | +3 por rango a T.I. cuando se usa para romper objetos |

---

## Mazas

Armas contundentes optimizadas para golpear a través de la armadura. Las mazas a menudo ignoran parte del Bloqueo o dan bonificadores contra adversarios específicamente protegidos.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Morgenstern | 1 | d8 | 1 m | 2 kg | Fuerza | Primaria | +1 por rango a T.I. contra enemigos con armadura intermedia |
| Nekhakha | 1 | d8 | 1 m | 2 kg | Agilidad | Primaria | +1 por rango a T.A. en reacciones o contraataques |
| Kanabo | 2 | d10 | 2 m | 5 kg | Tenacidad | Primaria | Reduce el umbral de ruptura en 1 al intentar romper partes, no solo con impacto crítico |
| Shillelagh | 1 | d6 | 1 m | 1,5 kg | Sabiduría | Primaria | +1 por rango a T.A. en maniobras defensivas o desarmes |
| Shishpar | 1 | d8 | 1 m | 2,5 kg | Astucia | Primaria | Ignora 2 puntos de Bloqueo por rango contra enemigos desequilibrados, tambaleantes o derribados |

---

## Hojas Largas

Espadas de hoja larga, optimizadas para el corte sostenido. Equilibrio entre daño, alcance y versatilidad. Las versiones a dos manos aumentan el dado de daño pero requieren ambas manos.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Katana | 1 | d8 | 1 m | 1,2 kg | Agilidad | Primaria | +1 por rango a T.I. cuando se empuña con ambas manos |
| Spatha | 1 | d8 | 1 m | 1 kg | Fuerza | Primaria | +1 por rango a T.I. cuando se usa con escudo |
| Khopesh | 1 | d8 | 1 m | 1,5 kg | Tenacidad | Primaria | Ignora 1 punto de armadura por rango contra escudos |
| Shamshir | 1 | d8 | 1 m | 1 kg | Agilidad | Primaria | +1 por rango a T.A. si el portador se ha movido en esta misma activación |
| Claymore | 2 | d12 | 2 m | 2,5 kg | Tenacidad | Primaria | +1 por rango a T.I. contra múltiples enemigos |
| Mandoble | 2 | d10 | 2 m | 2 kg | Fuerza | Primaria | +1 por rango a T.A. en un ataque de carga |
| Estoque | 1 | d8 | 1 m | 1,1 kg | Astucia | Primaria | Ignora 2 puntos de Bloqueo por rango al golpear puntos vitales |
| Schiavona | 1 | d8 | 1 m | 1,3 kg | Sabiduría | Primaria | +1 por rango a T.A. en maniobras combinadas con Engaño o Acrobacias |

---

## Dagas

Armas cortas de uso auxiliar. Las dagas destacan en ataques furtivos, inmovilizaciones y defensa activa, y son el complemento natural para combate con dos armas.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Sai | 1 | d4 | 1 m | 0,7 kg | Agilidad | Auxiliar | +1 por rango a T.D. cuando se desvían o bloquean ataques cuerpo a cuerpo |
| Jutte | 1 | d4 | 1 m | 0,6 kg | Sabiduría | Auxiliar | +2 por rango a T.A. para desarmes o inmovilizaciones |
| Scian | 1 | d4 | 1 m | 0,5 kg | Agilidad | Auxiliar | +2 por rango a T.I. desde posición oculta o furtiva |
| Kris | 1 | d4 | 1 m | 0,6 kg | Astucia | Auxiliar | +1 por rango a T.A. contra enemigos con heridas activas |

---

## Hojas Cortas

Espadas cortas con mayor alcance y dado que las dagas. Pueden usarse como arma primaria o como auxiliar en combate con dos armas según el modelo.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Wakizashi | 1 | d6 | 1 m | 0,9 kg | Agilidad | Auxiliar | +1 por rango a T.A. en combate con dos armas |
| Tanto | 1 | d4 | 1 m | 0,6 kg | Agilidad | Auxiliar | +1 por rango a T.I. desde posición oculta o furtiva |
| Kama | 1 | d6 | 1 m | 0,7 kg | Agilidad | Auxiliar | +1 por rango a T.I. en el segundo ataque de una misma activación |
| Claideamh | 1 | d6 | 1 m | 0,8 kg | Fuerza | Auxiliar | +1 por rango a T.A. contra enemigos con armadura ligera |
| Seax | 1 | d4 | 1 m | 0,7 kg | Fuerza | Auxiliar | +1 por rango a T.A. en maniobras de área contra un solo objetivo |
| Cimitarra | 1 | d8 | 1 m | 1,1 kg | Agilidad | Primaria | +1d2 de daño adicional por rango cuando el enemigo falla su T.D. |
| Akinakes | 1 | d4 | 1 m | 0,7 kg | Agilidad | Auxiliar | Permite un ataque adicional con la mano libre una vez por activación en combate con dos armas |
| Xiphos | 1 | d4 | 1 m | 0,8 kg | Agilidad | Auxiliar | +1 por rango a T.E. de Destreza durante el combate con dos armas |

---

## Armas Arrojadizas

Armas diseñadas para atacar a distancia mediante lanzamiento. Algunas pueden usarse en serie durante la misma activación. El Dory de la familia de lanzas también puede arrojarse con bonificador.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Kunai | 1 | d4 | 10 m | 0,3 kg | Agilidad | Auxiliar | +1 por rango a T.A. cuando se lanza a la mitad del alcance máximo |
| Shuriken | 1 | d4 | 10 m | 0,2 kg | Agilidad | Auxiliar | +1 por rango a T.A. cuando se usa en serie durante la misma activación |
| Pilum | 1 | d6 | 10 m | 1,5 kg | Fuerza | Primaria | +2 por rango a T.I. cuando se lanza para atravesar escudos o defensas |
| Francisca | 1 | d6 | 10 m | 1,4 kg | Fuerza | Primaria | +1 por rango a T.I. en cargas o ataques de avance |
| Chakram | 1 | d4 | 12 m | 0,4 kg | Astucia | Auxiliar | +1 por rango a T.A. cuando se usa contra más de un objetivo en el mismo arco |

---

## Armas a Distancia

Armas que disparan proyectiles. El dado de daño depende del tipo de proyectil usado, no del arma en sí.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Yumi | 2 | Por proyectil | 30 m | 1 kg | Agilidad | Primaria | +1 por rango a T.A. contra objetivos a más de 15 metros |
| Gakgung | 2 | Por proyectil | 20 m | 1 kg | Agilidad | Primaria | +1 por rango a T.A. desde posición oculta o furtiva |
| Fukiya | 1 | Por proyectil | 20 m | 0,5 kg | Agilidad | Primaria | +1 por rango a T.I. en ataques de precisión contra puntos vitales |
| Scythian | 2 | Por proyectil | 30 m | 1 kg | Fuerza | Primaria | +1 por rango a T.A. cuando se dispara desde una montura |
| Balearic | 1 | Por proyectil | 20 m | 0,7 kg | Agilidad | Primaria | +1 por rango a T.A. por cada ataque consecutivo realizado en la misma activación |
| Sumpit | 1 | Por proyectil | 20 m | 0,5 kg | Agilidad | Primaria | +1 por rango a T.I. cuando el proyectil aplica veneno o enfermedad |

---

## Armas Flexibles

Armas con partes articuladas, cadenas o materiales que permiten golpear desde ángulos inusuales, envolver extremidades o generar presión a distancias intermedias.

| Arma | M | Daño | Alcance | Peso | Característica | Asignación | Bonificador |
| --- | :---: | :---: | --- | --- | --- | --- | --- |
| Kusarigama | 2 | d6 / d4 | 1–3 m | 1,5 kg | Agilidad | Primaria | +1 por rango a T.A. al alternar entre hoz y cadena en secuencias de múltiples ataques |
| Scourge | 1 | d4 | 1–3 m | 1 kg | Fuerza | Auxiliar | +1 por rango a T.A. contra enemigos a más de 1 metro |
| Nekode | 1 | d4 | 1 m | 0,5 kg | Agilidad | Auxiliar | +1 por rango a T.A. en Técnicas de los perfiles de Interrupción o Impredecible |
| Kusari Fundo | 2 | d6 | 1–3 m | 1,2 kg | Astucia | Primaria | +1 por rango a T.I. cuando el objetivo ya está desequilibrado, tambaleante o derribado |
| Spider Gloves | 1 | d4 | 1–3 m | 0,4 kg | Agilidad | Auxiliar | +1 por rango a T.E. de Trepar o Agarre cuando se usa para escalar, enganchar o bloquear a presión cercana |
| Urumi | 1 | d6 | 1–4 m | 1,6 kg | Agilidad | Primaria | Ignora 1 punto de Bloqueo por rango cuando se ataca desde más de 1 metro |
