...
The Rolling System defines the resolution mechanics for the actions characters attempt to carry out, whether in combat, during exploration, or in social interactions. Its purpose is to determine the success or failure of an action and, in certain cases, allow progression of associated competencies.

In general terms, checks use a `d10` and are modified by characteristics, competencies, equipment, and circumstances. Players can choose how to approach certain actions through Evolutionary Advantage, while NPCs and creatures use a simplified system aligned with their narrative function.

Competencies directly influence several checks in the system. For this reason, roll types are presented first, and later, in the Competencies section, how bonuses are gained and how progression works is explained.
...
The player rolls two dice and chooses the higher result to resolve the action. By doing this, they give up any learning opportunity derived from that check.

This option represents an approach oriented exclusively toward results: the character prioritizes immediate success over technical refinement.
...
This option represents a more careful and reflective execution: the character assumes a greater risk of failure in exchange for learning from experience.

## NPCs and Other Creatures

NPCs and monsters do not necessarily follow the same progression rules as player characters. Instead, they have Traits, which represent advantages, disadvantages, or special capabilities tied to their nature, behavior, environment, emotional state, or combat role.
...
In all cases, competencies fulfill two functions:

- they modify rolls
- they allow progression as the character puts them to the test in relevant situations

In the system, competencies do not replace characteristics: both complement each other. Characteristics express the character's potential; competencies express how much they have trained, practiced, or refined that potential.
...
Additionally, defensive and resistance competencies can progress when facing threats appropriate to their level, depending on competency type.
...
- Reaching Expert reduces movement speed penalty by one category.
- Reaching Master reduces that penalty by one additional category.
...
- Each competency rank grants `+1` to Affliction R.R. during cognitive meditation.
...
To avoid confusion, this is the base relationship between both systems:

- A.R. uses the competency of the weapon or object used.
- D.R. uses Evasion competency.
- I.R. primarily uses weapon competency rank.
- C.R. does not necessarily depend on competencies, unless a specific rule states otherwise.
- R.R. uses the resistance corresponding to the effect type.
- S.R. uses the competency of the involved specialization.
- P.R. does not use competencies.
