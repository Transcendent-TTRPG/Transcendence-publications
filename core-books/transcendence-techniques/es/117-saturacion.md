---
title: "Saturación"
type: corebook
content_kind: rules
writing_mode: rules
language: es
chapter: 9
status: draft
canonical: false
tags: [techniques, innate, species, yacani, active, utility, metabolic, alteration, removal]
authority_refs:
  - Transcendence-design/docs/system/techniques.md
  - Transcendence-design/data/system/techniques.yaml
related:
  - core-books/transcendence-corebook/09-techniques/es/01-superficie-de-juego-y-ejemplo-piloto.md
  - core-books/transcendence-corebook/06-species/es/20-yacani.md
---

# Saturación

### Activo - Utilidad

**Rango Novato**

*Los eritrocitos llevan el límite hasta donde no hay marcha atrás.*

| Rango | Área | Duración | Tirada |
| --- | --- | --- | --- |
| Personal | Tú | Instantáneo | `T.C. (Tenacidad)` |

| Salvación | Impacto | Ritmo | Desgaste |
| --- | --- | --- | --- |
| `—` | `—` | `2` | `3` |

## Requisitos

- Especie: Yacani
- Sufrir una Alteración física activa

## Keywords

- `Yacani`

## Efecto

Realizas una T.C. de Tenacidad contra la severidad de la Alteración activa que estás sufriendo. Si tienes éxito, la eliminas. No aplica a Aflicciones ni a Venenos — solo a Alteraciones de origen físico.
